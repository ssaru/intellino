...
class NeuronCells:
    ...
    def inference(self, vector: np.ndarray) -> np.ndarray:
        ...
        """
        k = 3
        "pre_min_distance = -1 for the case of the minimum distance is 0"
        pre_min_distance = -1

        for i in range(k):
            min_distance = np.min(np.array(distances)[np.array(distances)>=pre_min_distance])
            closest_cell_idx = distances.index(min_distance)

            "make minimum distance to 65535.0 for the case that same distance value selected"
            distances[closest_cell_idx] = 65535.0

            sorted_distances.append(min_distance)
            sorted_category.append(self.cells[closest_cell_idx].target)
        ...
        for i in range(k):
            if vote_cnt == 0:
                vote_cat = sorted_category[i]
                vote_cnt = 1
            else :
                if(sorted_category[i] == vote_cat) :
                    vote_cnt += 1
                else :
                    vote_cnt -= 1
