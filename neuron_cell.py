...
class NeuronCells:
    ...
    def inference(self, vector: np.ndarray, top_k: int) -> np.ndarray:
        """
        inference(classify)
        ...
        top_k_distance_indexes = sorted_distance_index[0:top_k]

        candidates = []
        for index in top_k_distance_indexes:
            candidates.append((distances[index], self.cells[index].target))

        vote_dict = {"count": 0, "category": -1}
        for distance, category in candidates:
            if vote_dict["count"] == 0:
                vote_dict["category"] = category
                vote_dict["count"] += 1
            else:
                if category == vote_dict["category"]:
                    vote_dict["count"] += 1
                else:
                    vote_dict["count"] -= 1

        return vote_dict["category"]
